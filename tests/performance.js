...
function main(target, test_name, plain_output){
    if (!test_name)
        test_name = 'large';
    if (!plain_output) console.log('Testing performance:', target);
    ...
    try {
        ...
        if (!plain_output) console.log('Performance: '+performance+' (lower is better)');
        process[plain_output].write(performance.toString());
    } catch(e){
